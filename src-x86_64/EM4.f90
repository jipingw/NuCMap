...
subroutine EM4(LcN,cN,LgF,gF,LwF,wF,LcF,cF,pattern,ind,index)

  implicit none
  integer  i,j,k,l,m,n,z,rd,wd,ind,tep,step,index
  integer  LcN,LgF,LwF,LcF,tp(0:500),id(0:500)
  real*8   pattern(4,8),lam(-2:7,4),temp,olik,lik,sw,sc,s1,s2,tmp,thd1,thd2,av,sd
  ...
  integer*1,allocatable:: iw(:),ic(:)
  real*8,allocatable:: O(:),Q(:),r(:),lamW(:),lamC(:),tw(:),tc(:)
  character*1,allocatable:: a(:); ind=0

  !---------- read genome to determine types 1234
  ...
  rewind(1); z=k*(n-1)+m
  allocate(iw(z),ic(z),a(z))
  read(1,*) tpc
  do i=1,z/k; read(1,'(500a1)') a((i-1)*k+1:i*k); end do
  if(z/k*k<z) read(1,'(500a1)') a(z/k*k+1:z)
  close(1); iw=1; ic=1
  do i=4,z-3
     if((a(i-3)=='A'.or.a(i-3)=='a').and.(a(i+3)=='T'.or.a(i+3)=='t')) then
        iw(i)=1; ic(i)=1
     else if((a(i-3)=='A'.or.a(i-3)=='a').and.(a(i+3)/='T'.and.a(i+3)/='t')) then
        iw(i)=2; ic(i)=3
     else if((a(i-3)/='A'.and.a(i-3)/='a').and.(a(i+3)=='T'.or.a(i+3)=='t')) then
        iw(i)=3; ic(i)=2
     else if((a(i-3)/='A'.and.a(i-3)/='a').and.(a(i+3)/='T'.and.a(i+3)/='t')) then
        iw(i)=4; ic(i)=4
     end if
  end do
  deallocate(a)
  ...
  close(1)
  open(1,file=cF,action='read'); cutC=0
  do; read(1,*,iostat=l) tepc,i,k; if(l/=0) exit
  ...
  lam=0.0; do k=1,4; lam(-2:1,k)=pattern(k,1:4); lam(4:7,k)=pattern(k,5:8); end do
  temp=sum(lam(:,1)); do k=1,4; lam(:,k)=lam(:,k)/temp; end do
  temp=(sum(cutW)+sum(cutC))/2.0d0/z; temp=temp/lam(6,1)
  lam=temp*lam
  ...
  do i=1,z
     ...
     do k=1,8; j=id(k)
        if(1<=i-j.and.i-j<=z) tw(i)=tw(i)+O(i-j)*lam(j,iw(i-j))
        if(1<=i+j.and.i+j<=z) tc(i)=tc(i)+O(i+j)*lam(j,ic(i+j))*r(i+j)
     end do
  ...
  do i=1,z
     ...
     do k=1,8; j=id(k)
        if(1<=i+j.and.i+j<=z) then
           sw=sw+cutW(i+j)*Q(i)*lam(j,iw(i))/tw(i+j); s1=s1+lam(j,iw(i))
        end if
        if(1<=i-j.and.i-j<=z) then
           sc=sc+cutC(i-j)*Q(i)*r(i)*lam(j,ic(i))/tc(i-j); s2=s2+r(i)*lam(j,ic(i))
        end if
     end do
  ...
  if(index==1) then; tepc='Chromosome'
     open(1,file='NCPscore.ratio_4temp.txt',status='replace')
     write(1,'(a,3x,a8,2x,a8,6x,a5,5x,a9)') tepc(1:LcN),'Position','NCPscore','Ratio','cNCPscore'
  else
     open(1,file='NCPscore.ratio_4temp.txt',position='append')
  end if
  do i=1,z
  ...
  deallocate(cutW,cutC,O,Q,r,lamW,lamC,tw,tc,iw,ic)
end subroutine EM4
